...
from dataset import ssd_augmentations_yolo
...
class ATR_sky_Dataset(Dataset):
	def __init__(self, list_path, labels_path, img_size, is_training, is_debug=False, batch_size=16):
		...
		self.total_transforms = ssd_augmentations_yolo.Compose(transforms=[])

		# self.transforms.add(data_transforms.KeepAspect())

		if is_training:
			self.total_transforms.add(ssd_augmentations_yolo.SSDAugmentation())
			if img_size is None:
				self.total_transforms.add(ssd_augmentations_yolo.ResizeImage_multi_scale(batch_size=self.batch_size))
			else:
				self.total_transforms.add(ssd_augmentations_yolo.ResizeImage_single_scale(new_size=img_size))
		else:
			self.total_transforms.add(ssd_augmentations_yolo.ToAbsoluteCoords())
			self.total_transforms.add(ssd_augmentations_yolo.xywh_to_xyxy())
			self.total_transforms.add(ssd_augmentations_yolo.ResizeImage_single_scale(new_size=img_size))
		self.total_transforms.add(ssd_augmentations_yolo.xyxy_to_xywh())
		self.total_transforms.add(ssd_augmentations_yolo.ToPercentCoords())
		self.total_transforms.add(ssd_augmentations_yolo.ToTensor(self.is_debug))
