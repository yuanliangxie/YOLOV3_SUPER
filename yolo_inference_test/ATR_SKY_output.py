...
if __name__ == '__main__':
	...
	#from yolo_inference_test.yolov5_config_ATR_SKY import TEST as config
	from yolo_inference_test.centernet_config_ATR_sky_test import TEST as config

	detector = yolo_inference_detector(config)
	img = cv2.imread('/media/xyl/6418a039-786d-4cd8-b0bb-1ed36a649668/Datasets/sky_challenge_competition/Images/160/005.bmp')
	t0 = time.time()
	...
	#输出单个图片目标的坐标
	bboxes_prd_xywh = xyxy2xywh(bboxes_prd)
	num_bboxes = len(bboxes_prd_xywh)
	num = 0
	while num < num_bboxes:
		x_car = bboxes_prd_xywh[num][0]
		y_car = bboxes_prd_xywh[num][1]
		print('object：'+str(num)+' ',x_car,y_car)
		num += 1
