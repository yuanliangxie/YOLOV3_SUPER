...
from evaluate.evaluate_ATR_sky_data.utils_voc import *
from evaluate.evaluate_ATR_sky_data.visualize import visualize_boxes
...
class Evaluator(object):
	def __init__(self, model, config, visiual=True):
		...
		self.pred_result_path = os.path.join(config['PROJECT_PATH'], 'evaluate/evaluate_ATR_sky_data/data', 'results')
	...
	def initial_generator(self):
		dataset = ATR_sky_Dataset(list_path=self.config["test_path"], labels_path=self.config["test_labels_path"], img_size=(self.val_shape, self.val_shape),
