...
from evaluate.evaluate_detrac_coco_api.utils_detrac import *
from evaluate.evaluate_detrac_coco_api.visualize import visualize_boxes
...
class Evaluator(object):
    def __init__(self, model, config, visiual=True):
        ...
        self.pred_result_path = os.path.join(config['PROJECT_PATH'], 'evaluate/evaluate_detrac_coco_api/data', 'results')
    ...
    def initial_generator(self):
        # dataset = VOCDataset(list_path=self.test_path, img_size=(self.val_shape, self.val_shape),
        #                      is_training=False, is_debug=False, batch_size=16 )
        dataset = DetracDataset(list_path=self.config["test_path"], ignore_region_path=self.config["test_ignore_region"],
                      labels_path=self.config["test_labels_path"], img_size=(self.val_shape, self.val_shape), is_training=False)
