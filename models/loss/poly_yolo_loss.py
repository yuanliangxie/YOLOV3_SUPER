...
class YOLOLoss(nn.Module):
	...
	def get_anchors_mask(self):
		#这里已经被修改，因为self.anchors与config_anchor是一样的，因为poly-YOLO在同一层上使用了所有的anchor
		mask = []
	...
	def forward(self, input, targets=None):
		...
		if targets is not None:
			...
			# print(
			#     'loss:{:.2f},loss_x:{:.5f},loss_y:{:.5f},loss_w:{:.5f},loss_h:{:.5f},loss_conf:{:.5f},loss_positive_conf:{:.5f}, loss_false_conf:{:.5f}, loss_cls:{:.2f}'.format(
			#         loss, loss_x * self.lambda_xy, loss_y * self.lambda_xy, loss_w * self.lambda_wh,
			#               loss_h * self.lambda_wh, loss_conf * self.lambda_conf, positive_loss, false_loss, loss_cls * self.lambda_cls
			#     ))
	...
	def get_target(self, target, scaled_anchors, in_w, in_h, pred_boxs, scaled_total_anchors):
		...
		for b in range(bs):
			...
			for t in range(target.shape[1]):
				...
				anchor_shapes = torch.FloatTensor(np.concatenate((np.array([0, 0]*len(scaled_total_anchors)).reshape(len(scaled_total_anchors), 2),
