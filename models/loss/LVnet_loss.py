...
class LVnetLoss(nn.Module):
	...
	def forward(self, input, targets=None):
		...
		if targets is not None:
			all_n_obj, mask, noobj_mask, tx, ty, tw, th, tconf, coord_scale, giou_gt_box= self.get_target(targets, scaled_anchors,
			...
			if self.config["GIOU"]:
				loss_giou = self.giou_loss.cal_giou_loss(giou_gt_box, pred_boxes, mask).sum()/all_n_obj
			else:
			...
			# loss_conf = ((1-conf) * mask * self.bce_loss(conf * mask, mask)).sum()/all_n_obj + \
			...
			# Results
			pred_cls = torch.ones_like(conf, requires_grad=False)
	...
	def get_target(self, target, scaled_anchors, in_w, in_h, pred_boxs, scaled_total_anchors):
		...
		for b in range(bs):
			...
			for t in range(target.shape[1]):
				...
				anchor_shapes = torch.FloatTensor(np.concatenate((np.array([0, 0]*4).reshape(4, 2),
		...
		return n_obj, mask, noobj_mask, tx, ty, tw, th, tconf, scales, giou_gt_box
