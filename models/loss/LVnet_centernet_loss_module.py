from models.loss.LVnet_centernet_loss import centernet_Loss
...
class centernet_loss_module(nn.Module):
	...
	def forward(self, input, target=None, n_obj=None):
		result = []
		result.append(self.centernet_loss(input, target, n_obj))
		return result
