...
from models.head.tiny_yolov3_head import tiny_yolov3_head
from models.backbone.tiny_yolov3.tiny_yolov3_backbone import tiny_yolov3_backbone
from models.backbone.tiny_yolov3.neck import tiny_neck
import models.loss.tiny_yolo_loss.tiny_yolov3_loss_module as loss
...
class tiny_yolov3(nn.Module):
	def __init__(self, config, logger=None, init_weight=True):
		super().__init__()
		self.backbone = tiny_yolov3_backbone()
		self.neck = tiny_neck()
		self.head = tiny_yolov3_head(nAnchors=3, nClass=config["model"]["classes"])
		self.loss = loss.yolo_loss_module(config, strides=[16, 32])
		if logger == None:
