...
from models.loss.LVnet_centernet_loss_module import centernet_loss_module
...
from models.loss.LVnet_loss_module_3 import LVnetloss_module
from models.assign_target.LVnet_assign_target import assign_targets
...
class LVnet(nn.Module):
	def __init__(self, config, logger=None, init_weight=True):
		...
		#self.neck = neck()
		self.head1 = head(in_channels=96, out_channels=64, nClass=config["model"]["classes"])
		self.head2 = head(in_channels=96, out_channels=64, nClass=config["model"]["classes"])
		self.head3 = head(in_channels=192, out_channels=128, nClass=config["model"]["classes"])
		self.head4 = head(in_channels=256, out_channels=256, nClass=config["model"]["classes"])

		self.smooth = nn.Sequential(
			SPP(),
			Conv2dBatchRelu6(1024, 64, kernel_size=1, stride=1, padding=0),
			Conv2dBatchRelu6(64, 256, kernel_size=3, stride=1, padding=1)
		)

		self.deconv4 = DeConv2d(256, 64, ksize=4, stride=2)
		self.deconv3 = DeConv2d(192, 32, ksize=4, stride=2)
		self.deconv2 = DeConv2d(96, 32, ksize=4, stride=2)
