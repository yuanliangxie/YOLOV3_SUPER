...
from models.backbone.Large_LVnet.LV_net_largest import LV_Net_backbone as backbone
...
class LVnet(nn.Module):
	def __init__(self, config, logger=None, init_weight=True):
		...
		#self.neck = neck()
		self.head1 = head(in_channels=128, out_channels=128, nClass=config["model"]["classes"])
		self.head2 = head(in_channels=256, out_channels=128, nClass=config["model"]["classes"])
		self.head3 = head(in_channels=384, out_channels=256, nClass=config["model"]["classes"])
		self.head4 = head(in_channels=512, out_channels=256, nClass=config["model"]["classes"])

		self.smooth = nn.Sequential(
			SPP(),
			Conv2dBatchRelu6(2048, 256, kernel_size=1, stride=1, padding=0),
			Conv2dBatchRelu6(256, 512, kernel_size=3, stride=1, padding=1)
		)

		self.deconv4 = DeConv2d(512, 128, ksize=4, stride=2)
		self.deconv3 = DeConv2d(384, 128, ksize=4, stride=2)
		self.deconv2 = DeConv2d(256, 64, ksize=4, stride=2)

		self.conv3 = Conv2dBatchRelu6(384, 384, kernel_size=1, stride=1, padding=0)
		self.conv2 = Conv2dBatchRelu6(256, 256, kernel_size= 1, stride=1, padding=0)
		self.conv1 = Conv2dBatchRelu6(192, 128, kernel_size=1, stride=1, padding=0)
	...
	def forward(self, input, target=None):
		...
		f4 = self.smooth(f4)  #x32

		f3 = torch.cat([f3, self.deconv4(f4)], dim=1) #尝试用加法试试
		f3 = self.conv3(f3)   #x16

		deconv_f3 = self.deconv3(f3)

		f2 = torch.cat([f2, deconv_f3], dim=1)
		f2 = self.conv2(f2)   #x8

		deconv_f2 = self.deconv2(f2)

		f1 = torch.cat([f1, deconv_f2], dim=1)
		f1 = self.conv1(f1)  #x4
