...
class ResBlock(nn.Module):
	def __init__(self, input_channels, output_channels, middle_channels):
		super(ResBlock, self).__init__()
		self.conv2dRelu = nn.Sequential(
			SeparableConv2d(input_channels, middle_channels, kernel_size=3, stride=1, padding=1),
			nn.BatchNorm2d(middle_channels),
			nn.ReLU6(),
			SeparableConv2d(middle_channels, output_channels, kernel_size=3, stride=1, padding=1),
...
class LV_Net_backbone(nn.Module):
	def __init__(self):
		...
		self.c2 = nn.Sequential(
			SeparableConv2d(64, 128, kernel_size=3, stride=2, padding=1),
			nn.BatchNorm2d(128),
			nn.ReLU6(),
			#nn.MaxPool2d(2, 2),
		)
		...
		self.c11 = nn.Sequential(
			...
			Conv2dBatchRelu6(128, 128, kernel_size=3, stride=2, padding=1),
			#nn.MaxPool2d(2, 2),
		)
		...
		self.largepart = ResBlock(512, 512, 1024)
